...
# defining the setup of the output
    def __str__(self):
        output = "\nName: " + str(self.name) + \
...
# loading the database in order to make changes
Base.metadata.create_all(engine)

Session = sessionmaker(bind=engine)
session = Session()

# creating a system that calculates the difficulty of a recipe
...
# creating a system for the user to add a new recipe
def create_recipe():
    recipe_ingredients = []
...
# defining a system where the user can search for multiple ingredients and return the recipes with said ingredients
def search_by_ingredients():
    ...
    # removes duplicate ingredients from the list
        ...
        try:
            ingredient_searched_nber = input(
                "\nEnter the number that corresponds to the ingredient you'd like to select. You can enter several numbers. In this case, numbers are seperated by a space: "
            )
            ...
            print("An unexpected error occured. Make sure to select a number from the list.")
...
def edit_recipe():
    if session.query(Recipe).count() == 0:
        ...
        if recipe_id_for_edit not in recipes_id_list:
            print("Not in the ID list. Please try again.")
        else:
            print("Continue")
            recipe_to_edit = session.query(Recipe).filter(
                Recipe.id == recipe_id_for_edit
            ).one()

            print("\nWARNING: You are about to edit the following recipe: ")
            print(recipe_to_edit)
            column_for_update = int(input(
                "\nEnter the data you would like to update, 1. name, 2. cooking_time, 3. ingredients (select 1, 2, or 3): "
            ))
            updated_value = input("\nEnter a new value for your selection: ")
            print("Choice: ", updated_value)

            if column_for_update == 1:
                print("Update the name of the recipe")
